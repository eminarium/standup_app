...
RSpec.describe "Users", type: :request do
  ...
  describe "PATCH #update_me" do
    ...
    it 'renders :me of failure' do
      patch '/user/update_me', params: { id: user.id, user: { name: "John", email: nil }}
  ...
  describe "PATCH #update_password" do
    ...
    it 'renders :password of failure' do
      patch '/user/update_password', params: { id: user.id, user: { password: '123', password_confirmation: '123' } }
  ...
  describe "GET /new" do
    ...
    it "fails on unauthenticated user" do
      user = User.create!(name: "John", email: "specuseruser@test.com", password: 'spec123')
      user.add_role :user
      sign_in user
  ...
  describe "POST #create" do
    ...
    it "creates a user" do
      post '/account/users', params: { user: FactoryBot.attributes_for(:user, { role: 'user' }) }
      expect(response).to redirect_to account_users_path
    end

    it 'renders :new on failure' do
      post '/account/users', params: { user: FactoryBot.attributes_for(:user, { email: nil, role: 'user' }) }
      expect(response).to render_template(:new)
    end
    ...
    it "fails on non unique email" do
      ...
      post '/account/users', params: { user: FactoryBot.attributes_for(:user, { email: user.email, role: 'user' }) }
      expect(response).to render_template(:new)
    end
  end
  ...
  describe "PUT #update" do
    ...
    it "renders :edit on failure" do
      put "/account/users/#{user.id}", params: { id: user.id, user: FactoryBot.attributes_for(:user, { email: nil, role: 'user' }) }
      expect(response).to render_template(:edit)
    end
  end
